...
let db=load();function persist(){fs.writeFileSync(DB,JSON.stringify(db,null,2))}
...
async function api(req,res,url){
 ...
 if(req.method==='GET'&&p==='/api/console'){const u=requireUser(req,res,'buyer');if(!u)return;const incoming=db.lots.filter(l=>l.transaction_status==='open'),confirmedToday=db.lots.filter(l=>l.transaction_status==='completed'&&l.facility_id===u.facility_id).length;return json(req,res,200,{incoming,confirmedToday,facilities:db.facilities,traceability:db.lots.length?Math.round(db.lots.filter(l=>l.transaction_status==='completed').length/db.lots.length*100):0,buyer:publicUser(u)});}
